CREATE OR REPLACE PROCEDURE dw.SP_DW_VOLTE_QUALITY_SEQ_NE_D(
	  in v_day_key bigint
)
on exception resume
begin
    ...
    set v_sql_text = 'delete from dw.DW_VOLTE_QUALITY_SEQ_NE_D where DAY_KEY='||v_day_key||'';
   ...
   set v_sql_text ='
insert into DW_VOLTE_QUALITY_SEQ_NE_D
(
DAY_KEY
,NE_NAME
,S_CSCF_SUCC_RATE
...
from DW_VOLTE_QUALITY_SEQ_NE_H a
where a.DAY_KEY='||v_day_key||'
group by a.DAY_KEY,a.NE_NAME
