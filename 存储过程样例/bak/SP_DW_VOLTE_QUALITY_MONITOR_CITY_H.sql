CREATE OR REPLACE PROCEDURE dw.SP_DW_VOLTE_QUALITY_MONITOR_CITY_H(
	  in v_day_key bigint,
...
begin
    ...
    set v_sql_text = 'delete from dw.DW_VOLTE_QUALITY_MONITOR_CITY_H where DAY_KEY='||v_day_key||' and HOUR_KEY='||v_hour_key||'';
   ...
   set v_sql_text ='
insert into DW_VOLTE_QUALITY_MONITOR_CITY_H
(
DAY_KEY
,HOUR_KEY
,CITY_NAME
...
from 
    (select * from DW_VOLTE_QUALITY_OMC_CITY_H where day_key='||v_day_key||' and hour_key='||v_hour_key||') a
    full join
    (select * from DW_VOLTE_QUALITY_SEQ_CITY_H where day_key='||v_day_key||' and hour_key='||v_hour_key||') b
